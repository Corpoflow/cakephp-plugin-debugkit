...
class HtmlToolbarHelper extends ToolbarHelper {
    ...
    public function makeNeatArray($values, $openDepth = 0, $currentDepth = 0, $doubleEncode = false) {
        ...
        foreach ($values as $key => $value) {
            $current_li = '<strong>' . h($key, $doubleEncode) . '</strong>';
            ...
            if ( ! empty($value) && ($value instanceof ArrayAccess || $value instanceof Iterator || is_array($value) || $isObject)) {
                $has_next_level = true;
                $current_li .= $this->makeNeatArray($value, $openDepth, $nextDepth, $doubleEncode);
            } else {
                $current_li .= h($value, $doubleEncode);
            }

            $out .= $this->Html->tag('li', $current_li, ['class' => ($has_next_level) ? 'has-next-level' : null]);
        }
        $out .= '</ul>';
